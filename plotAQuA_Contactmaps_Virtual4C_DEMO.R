...
### 1.Set up project and samples. 
      ...
      setwd("K:/projects/HiC/projects/")   #Configure to adopt code into your own file system.
...
### 2. Obtain spike in read counts. Mergestat is a standard output from the HiC-pro pipeline
  ###GENERATION OF MERGESTAT DATAFRAME IS COMMENTED OUT FOR DEMO 
      ...
      # })
      # 
      # write.table(mergestat.all, "mergestat.HiChIP.all.txt", sep="\t", col.names = T, row.names = F)
      #
  ## download here: https://github.com/GryderArt/AQuA-HiChIP/blob/master/DEMO_data/mergestat.HiChIP.all.txt
  
      mergestat.all = read.table("mergestat.HiChIP.all.txt", sep="\t", header = T)
